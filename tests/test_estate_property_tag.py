...
class TestEstatePropertyTag(TransactionCase):
    ...
    def test_create_tag(self):
        tag_data = {"name": "TAG 001", "color": 99}
    ...
    def test_update_tag(self):
        tag = self.estate_property_tag_model.create({"name": "TAG 003", "color": 70})
    ...
    def test_update_with_invalid_value(self):
        with self.assertRaises(ValidationError) as context:
            tag = self.estate_property_tag_model.create(
                {"name": "TAG 003", "color": 70}
            )
            tag.write({"color": 150000})
            self.assertEqual(
                str(context.exception), "Color Index must be less than 100000"
            )

        # --------- test method N° 5 ------------

    def test_delete_tag(self):
        tag = self.estate_property_tag_model.create({"name": "TAG 010", "color": 70})
        tag.unlink()
