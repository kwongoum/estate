...
class EstatePropertyTagController(http.Controller):
    ...
    def get_property_tags(self, **kwargs):
        ...
        property_tags = []
        for tag in tags:
            property_tags.append({"id": tag.id, "name": tag.name, "color": tag.color})
    ...
    def get_property_tag_by_id(self, tag_id, **kwargs):
        try:
            ...
            response_data = {"id": tag.id, "name": tag.name, "color": tag.color}
            return Response(json.dumps(response_data), content_type="application/json")
        except Exception as e:
            return Response(
                json.dumps({"error": "something went wrong", "error_detail": str(e)}),
                status=500,
            )
    ...
    def create_tag(self, **kwargs):
        try:
            ...
            check_tag_exist = request.env["estate.property.tag"].search(
                [("name", "=", name)]
            )
            if check_tag_exist:
                return Response(
                    f"Tag with this name ({name}) is already exist!", status=400
                )
    ...
    @http.route(
        "/estate-property-tag/<int:tag_id>", auth="public", methods=["PUT"], csrf=False
    )
    @authenticate
    def update_tag(self, tag_id, **kwargs):
        try:
            ...
            tag.write({"name": name, "color": color})
            response = json.dumps({"id": tag.id, "name": name, "color": color})
            return Response(response, content_type="application/json")
        except Exception as e:
            return Response(f"There is error occured: {str(e)}", status=500)
    ...
    @http.route(
        "/estate-property-tag/<int:tag_id>",
        auth="public",
        methods=["Delete"],
        csrf=False,
    )
